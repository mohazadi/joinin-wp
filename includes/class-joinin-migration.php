...
class JoinIN_Migration {
	...
	private function import_rooms() {
		global $wpdb;
		$old_rooms_table     = 'wp_joinin';
		$old_room_logs_table = 'wp_joinin_logs';
		...
		if ( $wpdb->get_var( $old_rooms_query ) === $old_rooms_table ) {
			...
			foreach ( $old_rooms as $old_room ) {
				...
				if ( 0 === $new_room_id ) {
					$this->error_message = sprintf( wp_kses( __( 'Failed to import the room, %s.', 'joinin' ), array() ), $old_room->meetingName );
					return false;
				} else {
			...
			if ( count( $check_old_rooms ) > 0 ) {
				$this->error_message = __( 'Not all rooms were able to be imported to the new version.', 'joinin' );
				return false;
			} else {
	...
	private function import_permissions() {
		$old_permissions = get_option( 'joinin_permissions' );
		if ( false === $old_permissions ) {
			delete_option( 'joinin_permissions' );
			return;
		}
	...
	public function import_from_older_versions() {
		...
		// Set joinin_plugin_version value.
		update_option( 'joinin_plugin_version', '1.4.6' );
	...
	public function import_from_older_versions_rooms() {
		...
		$table_name = $wpdb->prefix . 'joinin';

		// Updates for version 1.3.1 and earlier.
		$joinin_plugin_version_installed = get_option( 'joinin_plugin_version' );
		if ( false === $joinin_plugin_version_installed || ( strcmp( '1.3.1', $joinin_plugin_version_installed ) <= 0 && get_option( 'bbb_db_version' ) ) ) {

			// Initialize database will create the tables added for version 1.4.6.
			$this->joinin_init_old_database();
			...
			foreach ( $list_of_meetings as $meeting ) {
				...
				$wpdb->query( $wpdb->prepare( $sql, JoinIN_Admin_Helper::generate_random_code( 6 ), $meeting->meetingID, $meeting->meetingVersion, $meeting->attendeePW, $meeting->moderatorPW ) );
			}
			...
			if ( ! get_option( 'mt_bbb_url' ) ) {
				update_option( 'joinin_url', 'https://api.joinin.ir/<user>/' );
			} else {
				update_option( 'joinin_url', get_option( 'mt_bbb_url' ) );
				delete_option( 'mt_bbb_url' );
			}

			if ( ! get_option( 'mt_salt' ) ) {
				update_option( 'joinin_salt', '90e101574e400318cd8ef521a665b55e' );
			} else {
				update_option( 'joinin_salt', get_option( 'mt_salt' ) );
	...
	public function import_from_older_versions_permissions() {
		...
		if ( $joinin_plugin_version_installed && strcmp( $joinin_plugin_version_installed, '1.3.3' ) < 0 ) {
			$roles              = $wp_roles->role_names;
			$roles['anonymous'] = 'Anonymous';
			$permissions        = [];
			if ( get_option( 'joinin_permissions' ) ) {
				$old_permissions = get_option( 'joinin_permissions' );
				...
				update_option( 'joinin_permissions', $permissions );
			}
		}
	}
	 ...
	 * @since   1.4.6
	 */
	public function joinin_init_old_database() {
		...
		// Sets the name of the table.
		$table_name      = $wpdb->prefix . 'joinin';
		$table_logs_name = $wpdb->prefix . 'joinin_logs';
        ...
        VALUES ('" . JoinIN_Admin_Helper::generate_random_code( 6 ) . "','Demo meeting', '" . time() . "', 'ap', 'mp');";
		dbDelta( $sql );

		$sql = 'INSERT INTO ' . $table_name . " (meetingID, meetingName, meetingVersion, attendeePW, moderatorPW, recorded)
        VALUES ('" . JoinIN_Admin_Helper::generate_random_code( 6 ) . "','Demo meeting (recorded)', '" . time() . "', 'ap', 'mp', TRUE);";
		dbDelta( $sql );
