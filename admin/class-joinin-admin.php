...
class JoinIN_Admin {
	...
	public function enqueue_styles() {
		 ...
		 * defined in JoinIN_Loader as all of the hooks are defined
		 * in that particular class.
		 *
		 * The JoinIN_Loader will then create the relationship
		 * between the defined hooks and the functions defined in this
		 * class.
		 */

		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/joinin-admin.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/joinin-admin.js', array( 'jquery' ), $this->version, false );
	...
	public function create_admin_menu() {
		add_menu_page(
			__( 'Rooms', 'joinin' ), __( 'Rooms', 'joinin' ), 'activate_plugins', 'bbb_room',
			'', 'dashicons-video-alt2'
		);

		if ( current_user_can( 'manage_categories' ) ) {
			add_submenu_page(
				'bbb_room', __( 'Rooms', 'joinin' ), __( 'Categories' ), 'activate_plugins',
				'edit-tags.php?taxonomy=bbb-room-category', ''
			);
		}

		add_submenu_page(
			'bbb_room', __( 'Rooms', 'joinin' ), __( 'Settings' ), 'activate_plugins',
	...
	public function add_custom_room_column_to_list( $columns ) {
		$custom_columns = array(
			...
			'token'          => __( 'Token', 'joinin' ),
			'moderator-code' => __( 'Moderator Code', 'joinin' ),
			'viewer-code'    => __( 'Viewer Code', 'joinin' ),
		);
	...
	public function display_room_server_settings() {
		...
		require_once 'partials/joinin-settings-display.php';
	}
	...
	public function fetch_room_server_settings() {
		$settings = array(
			'bbb_url'          => get_option( 'joinin_url', 'https://api.joinin.ir/<user>/' ),
			'bbb_salt'         => get_option( 'joinin_salt', '90e101574e400318cd8ef521a665b55e' ),
			'bbb_default_url'  => 'https://api.joinin.ir/<user>/',
			'bbb_default_salt' => '90e101574e400318cd8ef521a665b55e',
		);
	 ...
	 */
	public function joinin_show_upgrade_notification( $current_plugin_metadata, $new_plugin_metadata = null ) {
		if ( ! $new_plugin_metadata ) {
			$new_plugin_metadata = $this->joinin_update_metadata( $current_plugin_metadata['slug'] );
		}
	 ...
	 */
	private function joinin_update_metadata( $plugin_slug ) {
	 ...
	 *                          1 - success
	 *                          2 - bad url format
	 *                          3 - bad joinin settings configuration
	 */
	private function room_server_settings_change() {
		if ( ! empty( $_POST['action'] ) && 'bbb_general_settings' == $_POST['action'] && wp_verify_nonce( sanitize_text_field( $_POST['bbb_edit_server_settings_meta_nonce'] ), 'bbb_edit_server_settings_meta_nonce' ) ) {
			...
			update_option( 'joinin_url', $bbb_url );
			update_option( 'joinin_salt', $bbb_salt );

			if ( ! JoinIN_Api::test_joinin_server( $bbb_url, $bbb_salt ) ) {
				return 3;
			}
	...
	public function check_for_heartbeat_script() {
		...
		if ( ! wp_script_is( 'heartbeat', 'registered' ) && ! get_option( 'dismissed-' . $bbb_warning_type, false ) ) {
			$bbb_admin_warning_message = __( 'JoinIN works best with the heartbeat API enabled. Please enable it.', 'joinin' );
			$bbb_admin_notice_nonce    = wp_create_nonce( $bbb_warning_type );
			require 'partials/joinin-warning-admin-notice-display.php';
		}
	}
